...
#ifdef CONFIG_TARGET_DURIAN
		/* add by phytium for 64 mem*/
		u16 pref_base;
		unsigned int pref_base_up32;
		unsigned long limit;
		dm_pci_read_config16(dev, PCI_PREF_MEMORY_BASE, &pref_base);
		dm_pci_read_config32(dev, PCI_PREF_BASE_UPPER32, &pref_base_up32);
