...
#ifndef CONFIG_MACB_PHYTIUM_E2000
...
#endif
#ifdef CONFIG_MACB_PHYTIUM_E2000
	unsigned long		pclk;
...
static int macb_phytium_e2000_clk_init(struct udevice *dev, ulong rate)
{
	const char *pclk;
	struct macb_device *macb = dev_get_priv(dev);
...
#ifdef CONFIG_MACB_PHYTIUM_E2000
static int phytium_gem_sel_clk(struct macb_device *bp, int speed)
{
	u32 hs_ctrl = 0;

	if(bp->phy_interface == PHY_INTERFACE_MODE_USXGMII || bp->phy_interface == PHY_INTERFACE_MODE_XGMII){
		...
		if(speed == SPEED_2500){
			printf("macb sgmii speed 2500M!\n");
			speed = GEM_SPEED_2500;
			gem_writel(bp, SRC_SEL_LN, 0x0); /*0x1c04*/
			...
			printf("macb sgmii speed 100/10M!\n");
			speed = GEM_SPEED_100;
			gem_writel(bp, SRC_SEL_LN, 0x0); /*0x1c04*/
			gem_writel(bp, DIV_SEL0_LN, 0x4); /*0x1c08*/
			gem_writel(bp, DIV_SEL1_LN, 0x8); /*0x1c0c*/
	...
	hs_ctrl = gem_readl(bp, HSMAC);
	hs_ctrl = GEM_BFINS(HSMACSPEED, speed, hs_ctrl);
	gem_writel(bp, HSMAC, hs_ctrl);

	return 0;
}

static int phytium_gem_init_interface(struct macb_device *bp, int speed, int duplex)
{
	u32 ctrl = 0;

	phytium_gem_sel_clk(bp, speed);
	if (bp->phy_interface == PHY_INTERFACE_MODE_XGMII) {
		ctrl = macb_readl(bp, NCFGR);
		ctrl &=~ GEM_BIT(PCSSEL);
		...
		if(speed == SPEED_10000){
			...
			ctrl |= GEM_BIT(SERDES_RATE);
		}
		printf("init interface usxgmii: ctrl=0x%x", ctrl);
		gem_writel(bp, USX_CONTROL, ctrl);
		ctrl = gem_readl(bp, USX_CONTROL);
		ctrl &= ~(GEM_BIT(RX_SYNC_RESET));
		ctrl |= GEM_BIT(TX_EN);
...
#endif/*CONFIG_MACB_PHYTIUM_E2000*/
...
#ifdef CONFIG_DM_ETH
#ifdef CONFIG_MACB_PHYTIUM_E2000
			phytium_gem_init_interface(macb, SPEED_1000, duplex);
#endif
...
#ifdef CONFIG_DM_ETH
#ifdef CONFIG_MACB_PHYTIUM_E2000
		phytium_gem_init_interface(macb, SPEED_100, duplex);
#endif
...
#ifdef CONFIG_DM_ETH
#ifdef CONFIG_MACB_PHYTIUM_E2000
		phytium_gem_init_interface(macb, SPEED_10, duplex);
#endif
...
#else
	unsigned long macb_hz = macb->pclk;
#endif
#endif
...
#ifdef CONFIG_MACB_PHYTIUM_E2000
void e2000_eth_pad(void)
{
	int ret;

	printf("macb3 pad!\n");
	//pad, gsd_gmu_mdc_mac3, func5
	ret = readl((size_t)(0x32B30000 + 0x0210));
	ret |= (1<<0);
	ret &= ~(1<<1);
	ret |= (1<<2);
	writel(ret, (size_t)(0x32B30000 + 0x0210));
	//pad, gsd_gmu_mdc_mac3, func5
	ret = readl((size_t)(0x32B30000 + 0x0214));
	ret |= (1<<0);
	ret &= ~(1<<1);
	ret |= (1<<2);
	writel(ret, (size_t)(0x32B30000 + 0x0214));
}
#endif
...
static const struct macb_config phytium_e2000_config = {
	.caps = MACB_CAPS_GIGABIT_MODE_AVAILABLE | MACB_CAPS_JUMBO | MACB_CAPS_GEM_HAS_PTP | MACB_CAPS_BD_RD_PREFETCH,
	.dma_burst_length = 16,
	.hw_dma_cap = HW_DMA_CAP_32B,
	.clk_init = macb_phytium_e2000_clk_init,
	.usrio = &macb_default_usrio,
};
...
static const struct udevice_id macb_eth_ids[] = {
	...
	{ .compatible = "cdns,phytium-gem",
	  .data = (ulong)&phytium_e2000_config},
	{ }
};
